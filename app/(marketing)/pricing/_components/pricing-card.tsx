  ...
  return (
    <MaxWidthWrapper>
      <BentoGrid className="max-w-7xl  mx-auto md:auto-rows-auto gap-4 md:gap-8 lg:gap-12 border-b-4 border-[#f5f5f5]">
        <div className="flex flex-col col-span-4 border-b-2 border-[#F5F5F5] pb-12">
          <span className="col-span-4 text-start text-lg md:text-xl lg:text-2xl font-medium text-[#2d2d2d] mb-2 md:mb-4 max-w-xs md:max-w-sm lg:max-w-2xl">
            Pricing & plans
          </span>
        </div>
        <div className="flex flex-col col-span-4 justify-between">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-between gap-16 py-12">
            {PricingCard.filter((item) => item.isActive).map((item, i) => (
              <Card
                className={cn(
                  item.title === "Pro"
                    ? "w-full lg:w-[355px] bg-[#F5F5F5] border-none"
                    : "w-full lg:w-[355px] border-solid border-[#EBEBEB] lg:border-none"
                )}
                key={i}
              >
